...
fnew = sprintf("%s%s/DW%s_GPy.nc", Directory, Project,Flight)
...
cfs <- c(0., 21.335) 
cfs <- c(0.016, 21.9915)
cfs <- c(0.086, 21.9915)       # offset from circle studies
...
# apply circle-analysis corrections
circleDV <- 0.8
circleDH <- -0.06
print(sprintf("offsets of %.1f and %.1f being applied to TASX and THDG for wind calculation", circleDV, circleDH))
TASX <- TASX + circleDV
THDG <- THDG + circleDH
...
VEW[is.na(VEW)] <- 0.
VNS[is.na(VNS)] <- 0.
GGVEWB[is.na(GGVEWB)] <- 0.
GGVNSB[is.na(GGVNSB)] <- 0.
netCDFfile <- open.ncdf (fname, write=FALSE)
if ("sps25" %in% names(netCDFfile$dim)) {
  print("processing includes shifting GGVEWB and GGVNSB forward by 240 ms")
  n = 6
  NLG <- length(GGVNSB)
  GGVNSB <- c(GGVNSB[(1+n):NLG],rep(GGVNSB[NLG],n))
  GGVEWB <- c(GGVEWB[(1+n):NLG],rep(GGVEWB[NLG],n))
}
