% set flight number on line 212, at top of first R chunk
...
capnF1 <- "Angle of attack from fit (A) vs angle of attack from the radome (AKRD)"
...
<<AOA1,fig.cap="capnF1,out.width="300pt",fig.align="center">>=

# note: knitr on tikal doesn't accept multiple captions, so it is 
#       necessary to include repeated chunks for each plot
Cradeg <- pi / 180.
...
capn <- "Vertical wind obtained from the angle of attack in the previous figure used with conventional measurements of TASX, PITCH, and VSPD. Also shown are the normal archive-file measurements WI GP from the gust-probe system"
@


<<AOA2, fig.cap=capn, out.width="300pt", fig.align="center">>=
WIX <- TASX*(A-PITCH)*Cradeg + VSPD
plot (WIC, WIX, pch=16, col='blue', cex=0.5)
lines (c(-5.,5.), c(-5.,5.), lty=2, col='orange', lw=2)
#points (WIC, WIX, col='blue', pch=16, cex=0.5)
legend("bottomright", legend=c("WIX -- new calc", 
                    "1:1 reference line"),
...
<<AOA-calibration-1, fig.cap="Reference angle of attack (AOAREF) as a function of the radome-measured angle of attack (AKRD)", out.width="300pt", fig.align="center">>=

AOAREF <- CPITCH_GP - asin(CVSPD_GP/TASX) / Cradeg
plot (AKRD, AOAREF, pch=16, cex=0.5, col='blue')
@
<<AOA-calibration-2, fig.cap="5-parameter fit (A2) to the reference angle of attack (AOAREF)", out.width="300pt", fig.align="center">>=
...
@
<<AOA-calibration-3, fig.cap="1-parameter fit to the reference angle of attack (AOAREF)", out.width="300pt", fig.align="center">>=
fm3 <- lm(AOAREF~AQR)
cf <- coefficients (fm3)
...
<<TAS-from-the-gust-pod-1,fig.cap=c("Standard measurements QCPC/PSXC plotted vs QCPC-GP/PS-GP as measured by the gust pod",out.width="300pt", fig.align="center">>=
B1 <- QCXC/PSXC
B2 <- QC_GP/PS_GP
...
capn2 <-"True airspeed obtained from the gust pod, plotted against corresponding measurements TASX from the conventional measurement system" 
@
<<TAS-from-the-gust-pod-2,fig.cap=capn2, out.width="300pt", fig.align="center">>=
...
<<Plotted-Results-1, fig.cap=capn3, out.width="300pt", fig.align="center">>=
...
@
<<Plotted-Results-2, fig.cap=capn4, out.width="300pt", fig.align="center">>=
...
<<Plotted-Results-3, fig.cap=capn5, out.width="300pt", fig.align="center">>=
...
@
<<Plotted-Results-4, fig.cap=capn6", out.width="300pt", fig.align="center">>=
...
\begin{figure}
\noindent \begin{centering}
\includegraphics[width=10cm]{GPcalFig2}
\par\end{centering}

\protect\caption{\label{fig:WIG}\textsl{Comparison of measurements from the radome
gust system (WIC) and from the gust pod (WIG).}}


\end{figure}
The vertical wind obtained from (\ref{eq:wi}) is plotted in Fig.~\ref{fig:WIG}
against the corresponding measurements of vertical wind obtained from
the radome gust system, WIC. As calculated here, these measurements
are completely independent; all measurements entering the calculation
of WIG are provided by the gust pod, including the attitude angles,
true airspeed, and the vertical motion of the aircraft. The small
offset between the mean values from the two measurements (0.2 m/s)
could be removed by adding this value to measurements of WIG, and
there is some justification for this because the mean value of WIG
alone for several flights is consistently about -0.15 m/s while the
mean value for WIC is +0.05 m/s.

\begin{figure}
\noindent \begin{centering}
\includegraphics[width=12cm]{GPcalFig7}
\par\end{centering}

\protect\caption{\label{fig:WIGexample}\textsl{An example of the measurements from
the gust pod and from the radome-based system, for a flight leg in
DEEPWAVE flight rf05 (16 June 2014). }}


\end{figure}
An example of vertical wind measured using the two systems is shown
in Fig.~\ref{fig:WIGexample}

The preceding approach treats the gust pod as an independent system
that should be self-contained to the extent possible, although the
true-airspeed measurement was based on a fit to measurements from
the standard system to take advantage of the low-uncertainty prior
calibration of that system. Another approach could be to calculate
the vertical wind to match that from the radome system, and it would
be possible to obtain still lower variance between the measurements
from the two systems by following this approach. 
\begin{figure}
\noindent \begin{centering}
\includegraphics[width=10cm]{GPcalFig5}
\par\end{centering}

\protect\caption{\label{fig:WIXvsWIC}\textsl{The vertical wind obtained by fitting
angle-of-attack measurements from the gust pod to match those from
the radome and then using that fitted angle-of-attack with the conventional
measurement of true airspeed (TASX) to obtain the vertical wind, compared
to the conventional measurement (WIC). Data from the flight of 16
June 2014, DEEPWAVE rf05. Measurements restricted to roll within $\pm$10$^{\circ}$,
TASX>130 m/s, and rate of climb within $\pm$2 m/s. }}


\end{figure}
For example, Fig.~\ref{fig:WIXvsWIC} shows a comparison of the vertical
wind calculated in this way to the conventional measurement. The standard
deviation between the two measurements is 0.07 m/s, and so is significantly
lower than that shown in Fig.~\ref{fig:WIG}. However, the weakness
in this approach is that, in turns, the results are substantially
worse than those leading to WIG.


\subsection*{Horizontal Wind}

\begin{figure}
\noindent \begin{centering}
\includegraphics[width=10cm]{GPcalFig6}
\par\end{centering}

\protect\caption{\label{fig:HorizWindComparison}\textsl{Comparison between the wind
measurement based on the gust pod (WDG, WSG) and that based on the
radome gust system (WDC, WSC). }}


\end{figure}
Figure \ref{fig:HorizWindComparison} shows a comparison of the horizontal
wind measurements from the gust pod to the conventional wind measurements.
The standard deviations between measurements of wind direction and
wind speed are 3$^{\circ}$ and 1.0 m/s, respectively.

One problem that appears in the horizontal wind measurements is that
there are occasional spikes in the horizontal wind, usually of 1-s
duration. These occur when the heading moves through 180$^{\circ}$,
where for some reason the gust-probe IRU produces a jump in heading
of about 10$^{\circ}$. In the data examined, these all occur during
turns. It will be useful either to flag these as missing value points
or interpolate through the jumps in order to avoid these spikes, which
can be as large as 10 m/s in wind speed or very large angle changes
in wind direction. 


\subsection*{Variance Spectra}

\begin{figure}
\noindent \begin{centering}
\includegraphics[width=10cm]{GPcalFig8}
\par\end{centering}

\protect\caption{\label{fig:VarSpec}\textsl{Variance spectra for the vertical wind
based on the gust probe (WIG, blue line) and based on the conventional
radome-based gust system (WIC, dark green line). Both are smoothed;
the unsmooted spectrum for WIG is shown as the thin red line.}}


\end{figure}
The variance spectra for the two measurements of vertical wind look
quite similar, as shown in Fig.~\ref{fig:VarSpec}. It will be interesting
to repeat this plot for high-rate data, but at low rate there is little
difference between the spectra except that the conventional system
appears to show more clearly a high-frequency noise spectrum, as would
be expected at these low turbulence levels because the high-frequency
variations approach the resolution of the measuring system. The 858
sensor is much smaller and has shorter pressure lines, so it may be
that the high-frequency response will be better from that system.
...
There are several options to consider for processing:
\begin{enumerate}
\item Incorporate this code into nimbus. The processing is sample-by-sample,
so this would be possible, although the matrix multiplications need
to be coded unless existing code from gusto.c can be used for this
purpose.
\item Call an R script to do the processing sample-by-sample and return
the result to nimbus. This is probably impractical because of the
need to start and stop R each sample, unless the R options for compiling
scripts can be used. (I have never used this and don't know if it
is a practical option.)
\item Use a second-pass program to add the gust-probe variables, as I have
done. As an example, see the file /scr/raf\_data/DWrf05\_GP.nc, where
I have added variables named WIG, WDG, and WSG for the vertical wind,
the horizontal wind direction, and the horizontal wind speed. These
can be examined as for any netCDF file, using for example ncplot.
(I haven't added all the expected attributes yet to this file.) 
\end{enumerate}
For reference: The script I used is saved as \textasciitilde{}cooperw/RStudio/DEEPWAVE/GustPod.R,
accessible in EOL file space from machines like tikal. It reads a
netCDF file and adds the new variables \textbf{\textcolor{red}{\emph{to
the same file. }}}\textcolor{black}{It does attempt to avoid the problem
mentioned above that introduces spikes as the CMIGITS heading moves
through 180$^{\circ}$, but eventually this should be done better,
perhaps via interpolation. The R script needs some work also because
I didn't vectorize the angle transformations yet, in the interest
of getting this to work first.}



\vskip0.5truein \centerline{\textcolor{blue}{---- END OF THIS NOTE----}}
