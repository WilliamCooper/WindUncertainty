...
# nbest <- x[1]
# rmsmin <- x[2]
nbest <- -18        ## now means net of 3*25=75, close to THDG shift of -80
...
characterize and check the wind measurements. The example used here is
from flight 15, 3:38:30--3:54:30 UTC, as shown in Fig.~\ref{fig:CircleFlightPatterns}.
\noindent 
\begin{figure}[btp]
\begin{centering}
...
mneasured ground speed from the GPS relative to the IRU can be detected from
...
flown with good precision and symmetrically, so this is a good illustration of
what can be learned from the maneuver.
...
<<sine-plot, echo=FALSE, include=TRUE, fig.cap="Illustration of the expected variation in measured wind speed with flight angle relative to the wind direction, for assumed errors of $\\delta V=1$ m/s in true airspeed and $\\delta\\psi^{\\prime}=0.3^{\\circ}$ in heading and for an assumed true wind speed of 10 m/s. The assumed true airspeed is 155 m/s. The red arrows show the differences in measured wind speed between 90 and 270$^{\\circ}$ directions and between 0 and 180$^{\\circ}$ directions.">>=
...
\midrule
\midrule 
all turns & \Sexpr{format(round(bestWD,1), nsmall=1)} & \Sexpr{round(bestWS,1)} & \Sexpr{round(bestFit[1],1)} & \Sexpr{format(round(bestFit[4],2), nsmall=2)} & \Sexpr{format(round(rms,1), nsmall=1)}\tabularnewline
\midrule 
mean of measurements & \Sexpr{format(round(mwd,1), nsmall=1)} & \Sexpr{format(round(mws,1), nsmall=1)} &  &  & \tabularnewline
\midrule 
left turns & \Sexpr{format(round(bestWDL,1), nsmall=1)} & \Sexpr{round(bestWSL,1)} & \Sexpr{round(bestFitL[1],1)} & \Sexpr{format(format(round(bestFitL[4],2), nsmall=2), nsmall=1)} & \Sexpr{format(round(rmsL,1), nsmall=1)}\tabularnewline
\midrule 
left-turn measurements & \Sexpr{format(round(mwdL,1), nsmall=1)} & \Sexpr{round(mwsL,1)} &  & & \tabularnewline
\midrule 
right turns & \Sexpr{format(round(bestWDR,1), nsmall=1)} & \Sexpr{round(bestWSR,1)} & \Sexpr{round(bestFitR[1],1)} & \Sexpr{format(round(bestFitR[4],2), nsmall=2)} & \Sexpr{format(round(rmsR,1), nsmall=1)}\tabularnewline
\midrule 
right-turn measurements & \Sexpr{format(round(mwdR,1), nsmall=1)} & \Sexpr{round(mwsR,1)} &  & & \tabularnewline
\bottomrule
\end{tabular}
\par\end{centering}
...
There is an apparently significant difference of about 1~m/s in mean wind speed, both measured and resulting from the fits, between the right-turn circles and the left-turn circles. This is apparently a real difference between wind conditions in the regions where the circles were flown and is supported by the difference between maximum and minimum groundspeed for the two sets of circles. Conditions are more uniform for the rightn circles, as reflected in the lower residual error for those circles, so more weight will be given here to the results from the right-turn circles. Both indicate a small heading error, but that was obtained by imposing a shift forward for the ground-speed measurements GGVEW and GGVNS of 840~ms. Without this shift, the indicated heading error was -1.6$^{\circ}$ for the left-turn circles and $+1.5^{\circ}$ for the right-turn circles, a clear reflection of a timing error between the measurements from the inertial system and those from the GPS receiver. This finding was the reason that the shift was imposed in processing. \footnote{Except as it applies to this study, this shift normally has little effect because the ground speed used in the calculation of wind is the result of the complementary filter discussed in the next section so the measurements of ground speed from the GPS only enter with a filter time constant of about 120~s, while the short-term response is determined by the inertial system.} 
...
An appropriate error function to minimize to find values for the parameters $\delta V$, $\delta\xi$, and $v$ in \eqref{eq:delta-v-m} is $\chi_2^2=\sum{\delta v_m^2}$. 
...
\begin{tabular}{cccccc}
\toprule 
 &  $v$~{[}m/s{]} & $\delta V$~{[}m/s{]} & $\delta\xi\,[^{\circ}]$ & residual error {[}m/s{]}\tabularnewline
\midrule
\midrule 
all turns  & \Sexpr{round(cfcircleC[1],1)} & \Sexpr{round(VerrorC,1)} & \Sexpr{format(round(HerrorC,2), nsmall=2)} & \Sexpr{format(round(summary(fmcircleC)$sigma,1), nsmall=1)}\tabularnewline
\midrule 
left turns & \Sexpr{round(cfcircleL[1],1)} & \Sexpr{round(VerrorL,1)} & \Sexpr{format(format(round(HerrorL,2), nsmall=2), nsmall=1)} & \Sexpr{format(round(summary(fmcircleL)$sigma,1), nsmall=1)}\tabularnewline
\midrule 
right turns &  \Sexpr{round(cfcircleR[1],1)} & \Sexpr{round(VerrorR,1)} & \Sexpr{format(round(HerrorR,2), nsmall=2)} & \Sexpr{format(round(summary(fmcircleR)$sigma,1), nsmall=1)}\tabularnewline
\bottomrule
\end{tabular}
\par\end{centering}
...
<<first-circles, fig.lp="fig:", fig.cap="Measured wind speed from the left-turn and right-turn circles in the circle pattern shown in Fig.\\ \\ref{fig:CircleFlightPatterns}, as a function of $\\xi$, the difference between the heading and the mean wind direction. Orange lines are the results of fitting \\eqref{eq:delta-v-m} to the measurements, with results as listed in Table~\\ref{tab:Circlemin}.",fig.width=5.0, fig.height=3.5, echo=FALSE, hold=FALSE, include=TRUE>>=
...
<<second-circles, fig.lp="fig:", fig.cap="Measured wind speed from the left-turn and right-turn circles in the second circle pattern as a function of $\\xi$, the difference between the heading and the mean wind direction. Orange lines are the results of fitting \\eqref{eq:delta-v-m} to the measurements, with results as listed in Table~\\ref{tab:Circle2min}.",fig.width=5.0, fig.height=3.5, echo=FALSE, hold=FALSE, include=TRUE>>=
...
<<third-circles, fig.lp="fig:", fig.cap="Measured wind speed from the left-turn and right-turn circles in the third circle pattern  as a function of $\\xi$, the difference between the heading and the mean wind direction. Orange lines are the results of fitting \\eqref{eq:delta-v-m} to the measurements, with results as listed in Table~\\ref{tab:Circle3min}.",fig.width=5.0, fig.height=3.5, echo=FALSE, hold=FALSE, include=TRUE>>=
...
Some of the fits, notably the left-turn circles from the third maneuver, don't conform very well to the expected sinusoidal error pattern. The residual error about the fit was the largest of the circles, with the right-turn circles having almost as large a residual error, which may indicate that conditions were not sufficiently uniform to use these maneuvers. The average indicated error in airspeed from all six circle maneuvers was 0.2$\pm$0.3\,m/s, and the indicated error in adjusted heading $\xi$ was $-0.02\pm 0.09^{\circ}$. These are useful constraints on the uncertainty associated with these key contributors to uncertainty in measured wind.
...
<<sine-cosine-plot, fig.lp="fig:", fig.cap="Predicted dependence of measured wind speed on direction of flight relative to the wind direction, for assumed errors of $\\delta V=1$ m/s and $\\delta\\psi=0.3^{\\circ}$.", fig.width=5.0, fig.height=4,echo=FALSE, hold=FALSE, include=TRUE, eval=FALSE>>=
A1 <- 1.
A2 <- mtas*0.3*pi/180.
AngI <- (1:1440)/4.
...
sideslip.\footnote{The angle of attack is determined by calibration so that
contribution to $\xi$ is not included in the error term.} 
The sideslip calibration was determined in 
...
terms in the equation. Although three different altitudes were used for
...
<<plotSS, fig.lp="fig:", fig.cap = "Measurement of sideslip during the first circle maneuver from DEEPWAVE flight 15, with left-turn circles from 3:40:00 -- 3:46:00 UTC, followed by a straight segment and then right-turn circles 3:47:30 -- 3:53:40 UTC.", fig.width=5, fig.height=4, echo=FALSE, include=TRUE>>=
...
Z <- plotWAC(Ds$Time, Ds$SS, ylab="Sideslip")
...
in the sideslip. This is a forced result, though, because the sensitivity
...
<<sideslip-error-histogram, fig.cap="Distribution of measurements of sideslip error determined from (\\ref{eq:predicted-error-in-sideslip}) for all measurements in turns from the three circle maneuvers.", include=TRUE, echo=FALSE>>=
...
This result, indicating that \Sexpr{round(dbm, 3)}$^{\circ}$
...
The uncertainty in the mean of this result is very low, and this result is more general because it uses measured values of the pitch, so this value will be used to correct the sideslip offset. This indicates that the sideslip sensitivity coefficients should be \{$e_{0}$,\, $e_{1}$\} = \{\Sexpr{round(cfs[1]-dbm, 3)}, \Sexpr{round(cfs[2], 3)}\}. Because this result is dependent on the vertical wind being zero, an uncertainty of at least 0.06$^{\circ}$ should be assigned to the first coefficient to recognize that the vertical wind might typically be $\pm$0.1\,m/s in a region such as this.\footnote{The measured vertical wind was 0.1\,m/s over the period of these circles.}

This result was obtained from files where a heading offset of -0.08$^{\circ}$ had been imposed. 
The results of this circle analysis point to a combined error from heading and sideslip of $-0.02\pm 0.09^{\circ}$, so the indicated heading error 
calculated from \eqref{eq:dpsiprime} is $-0.09^{\circ}$, which nearly offsets
the correction applied previously. It therefore appears preferable to remove that heading offset.
...
\begin{enumerate}
\item The circle maneuvers indicate that the measured true airspeed (TASX) with current LAMS-based pressure corrections is accurate to within expected uncertainty. The fits indicate an error of 0.2$\pm 0.3$\,m/s, which is within the expected ($\pm$0.3\,m/s) uncertainty limits for TAS deduced using the LAMS calibration (\cite{CooperEtAl2014}).
\item The indicated error in sideslip is 0.08$^{\circ}$, so the first sensitivity coefficient should be reduced by this amount. That leads to sensitivity coefficients for the radome of  \{\Sexpr{round(cfs[1]+0.084, 2)}, \Sexpr{format(round(cfs[2],3), nsmall=3)}\}. Cf.\ the discussion in Section\ \ref{sec:Calibrations} and the summary section \ref{CalSSsummary} at the end of that section.
It appears reasonable to consider that the bias in this measurement may be
 about 0.06$^{\circ}$ because this is the variability among different circle maneuvers and also the sensitivity to 0.1\,m/s vertical wind during the
maneuvers.
\item The heading offset presently in processing should be removed. The evidence
from the circle fits is that the combined bias associated with the heading and sideslip offsets is 0.02$\pm 0.09^{\circ}$, which is reasonably consistent with the estimated uncertainty in heading as specified for the IRU. The heading and sideslip offsets are couplied, so the values indicated by the circle fits should be used together.
\item The ground-speed measurements GGVEW and GGVNS from the Novatel GPS need to coincide with the inertial measurements. An offset of 80~ms is used for heading, so a similar assumed delay should be used for GGVEW/GGVNS.
\end{enumerate}
