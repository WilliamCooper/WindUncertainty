...
\protect\caption{\label{fig:Variance-spectra-VSPD}Variance spectra {[}units: m$^{2}$s$^{-2}$,
...
part of the data archive for the document.} The thick blue trace shows the spectrum for the standard
vertical wind
...
\protect\caption{\label{fig:WIXHR1}Variance spectra for the vertical wind (here, WIX)
...
ADIFR assert a standard uncertainty of about 0.07 hPa. As sampled
and digitized by the data system the resolution is about 0.002 hPa
($\pm70\,$hPa for $2^{16}$ range digital encoding), which is
comparable to the sensor resolution, so noise arising from digitation
would be much less intense than the measured noise distribution. Thus the white-noise
spectrum is consistent with a random error of about the
specified uncertainty for the sensor.
...
\protect\caption{\label{fig:AltOscillationFMS}The GPS-measured altitude [m] during a flight
...
\protect\caption{\label{fig:pitchManeuvers}Vertical wind measured during pitch maneuvers.
