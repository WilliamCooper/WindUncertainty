...
# Ddelay$AKRD <- Ddelay$AKRD * 1.00
  Ddelay$PITCH <- Ddelay$PITCH - CorrectPitch(Ddelay)[, 1]
...
<<plot-shifted-vertical-wind, include=TRUE, fig.cap="Vertical wind measured during pitch maneuvers. The variable GGVSPD is the vertical speed imposed on the aircraft during the pitch maneuver, shown here after division by 10, and WIX shows the resulting measurement of vertical wind after introducing the 40\\,ms time shift discussed in the text. The dashed orange lines denote limits of $\\pm$0.5\\,m/s. Data from DEEPWAVE flight 15.", fig.height=3.5>>=

Ddelay$GGVSPD <- Ddelay$GGVSPD / 10
plotWAC(Ddelay[setRange(Ddelay$Time, STM,ETM), c("Time", "GGVSPD", "WIX")], 
        ylab='W [m/s]', legend.position=NA)
legend ('topright', legend=c('GGVSPD/10', 'WIX'), lwd=2, col=c('blue', 'darkgreen'), text.col=c('blue', 'darkgreen'), cex=0.7)
