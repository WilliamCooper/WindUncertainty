...
the \href{http://www.eol.ucar.edu/instruments}{EOL instrument web pages}
...
a collection system to detect the Doppler shift in light back-scattered
from aerosols. The transmitter optical components are mounted in an
under-wing pod, similar to that used for the gust pod, and like the
