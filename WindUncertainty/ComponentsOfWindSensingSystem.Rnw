...
digitally to 25\,Hz or 1\,Hz in processing.\footnote{The specifications indicate that the appropriate time lag to apply
...
unit is a Novatel OEM-3 differential
...
a Systron Donner C-MIGITS III GPS/INS MEMS Tactical Grade navigation
system (henceforth called the C-MIGITS IRU), which is mounted in the pod to be
...
The C-MIGITS IRU unit provides measurements of the attitude angles,
...
The C-MIGITS IRU, mounted with the gust pod, provides
...
in turns if the exclusions listed in the figure caption were not applied to the measurements from the gust pod. These are the result of a problem with the C-MIGITS IRU
...
\protect\caption{\label{fig:VarSpecWICandWIG}Variance spectra for WIC 
...
course of the flight via the C-MIGITS IRU Kalman filter.
\end{enumerate}
...
\protect\caption{Diagram of the LAMS. Light generated by
...
This figure appears in Applied Optics in the article by \citet{SpulerEtAl2011} and is used here with permission from the Optical Society of America.}\label{fig:LAMSschematic}
\end{figure}
\end{center}
...
\protect\caption{The four-beam configuration of the LAMS optical head as installed in a GV 
under-wing pod.}
...
gust pod the system incorporates a compact Systron Donner SDN500
GPS/INS MEMS Tactical Grade navigation system (henceforth called the
SDN500 IRU) 
to measure the attitude angles and ground speed of the pod.
...
reducing uncertainty in the wind measurements, as described in \cite{CooperEtAl2014} and later in this report.
...
The process for obtaining the beam pointing vectors from the SDN500 IRU  is essentially identical to the process above, except that the SDN500 IRU provides roll, pitch and heading.  It should be noted, as will be addressed later, that the
SDN500 IRU heading is not reliable when the device is stationary.
...
\toprule 
\textbf{Beam} & \textbf{Angle difference between} \\ \textbf{SDN500 IRU and Telescope Mount}\tabularnewline
\midrule
\midrule 
...
\item The processes provides redundant angle measurements (SDN500 IRU and the telescope mount).
...
\toprule 
\textbf{Beam} & \textbf{$\theta_L$ Telescope} & \textbf{$\theta_L$ SDN500 IRU} & \textbf{$\theta_L$ Laser Survey} & \textbf{$\phi_L$ Telescope} & \textbf{$\phi_L$ SDN500 IRU} & \textbf{$\phi_L$ Laser Survey}\tabularnewline
\midrule
\midrule 
...
\end{equation}
where $\vec{v}_c$ is the air velocity vector in the SDN500 IRU coordinate frame and $\vec{v}_L$ the air velocity vector in the LAMS relative beam coordinate frame.  To propagate error in the transformation matrix we reframe the problem by vectorizing the matrix such that Eq. \eqref{velL2C} becomes
...
\mathbf{V_L}=\left[\begin{array}{ccccccccc}
v_1 & v_2 & v_3 & 0 & 0 & 0 & 0 & 0 & 0 \\
0 & 0 & 0 & v_1 & v_2 & v_3 & 0 & 0 & 0 \\
0 & 0 & 0 & 0 & 0 & 0 & v_1 & v_2 & v_3 
\end{array}\right],
\end{equation}
...
With the SDN500 IRU INS, the air velocity measurements can be transformed into a global coordinate frame.  The transformation matrix is determined using $\mathbf{T_1}$, $\mathbf{T_2}$ and $\mathbf{T_3}$ from the roll, pitch and yaw reported by SDN500 IRU.  The analysis needed for this step is covered in Section \ref{sub:EarthRef}.
...
for $C=1.5$ (FIND REFERENCE XXX)\marginpar{XXX}, (\ref{eq:Uvariance}) results in estimates of
