...
\item A laser air motion sensor described by \cite{SpulerEtAl2011} combined
...
\subsubsection{Transformation to an Earth Reference Frame}
...
defined in \href{https://www.eol.ucar.edu/raf/Bulletins/bulletin23.html}{RAF Bulletin 23} Eqs. 2.5 and 2.6:
...
The primary measurement on the GV is that based on measurement of
...
and the characteristics of the sensors used, and the \href{http://www.eol.ucar.edu/instruments}{EOL instrument web pages}
provide additional information on these measurements.\marginpar{XXX}
\begin{table}
...
\midrule 
horizontal GV velocity components (VNS, VEW) & Laseref IV Model HG2001 GD03 & strap-down accelerometers & 2.1\,m\,s$^{-1}$  & 0.1\,m\,s$^{-1}$ with slom updating to GPS\tabularnewline
\midrule 
...
For the period 3:00:00--6:00:00 on DEEPWAVE flight 15, the mean difference
in pitch was 0.1$^{\circ}$ and the standard deviation in the difference
between measurements was also about 0.12$^{\circ}$, but if measurements
are restricted to times where the magnitude of the roll is less than
5$^{\circ}$ then the standard
deviation is only 0.017$^{\circ}$. This may be an indication that
...
pitch and enhanced standard deviation in turns likely arise from small mis-alignment of the units relative to
...
IV Model HG2001 GD03, with characteristics as listedin Table~\ref{tab:Radome-system-measurements}.
...
presented in more detail in a separate document, but at this level of
uncertainty the measurement of temperature will be shown to introduce negligible
uncertainty to the measurement of wind.
XXX \marginpar{XXX}


\subsubsection{Examples of measurements}
<<radome-plot-angles, fig.cap="Attitude angles pitch, roll, and heading as measured by three independent systems, the standard Honeywell IRU (blue lines), a duplicate backup Honeywell IRU (green lines), and the C-MIGITS IRU mounted in the gust pod (red line). All plot units are degrees. Data from DEEPWAVE flight 16 (4 July 2014).", include=TRUE>>=

Flight <- "rf16"
VarNames <- c("PITCH", "CPITCH_GP", "ROLL", "CROLL_GP", "PITCH_IRS2", "ROLL_IRS2")
VarNames <- c(VarNames, "THDG", "CTHDG_GP", "THDG_IRS2")
VarNames <- c(VarNames, "QCF", "QCFC", "PSF", "PSFC", "TASX")
VarNames <- c(VarNames, "ADIFR", "BDIFR", "AKRD", "SSRD")
VarNames <- c(VarNames, "GGVEW", "GGVNS", "VEW", "VNS", "GGVSPD", "VSPD", "GGALT")
VarNames <- c(VarNames, "WDC", "WSC", "WIC")
VarNames <- c(VarNames, "WD_GP", "WS_GP", "WI_GP", "TAS_GP")
DE <- getNetCDF (sprintf("%s%s/%s%s.nc", DataDirectory (), Project, Project, Flight), VarNames)
op <- par (mar=c(2,4,1,1)+0.1)
layout(matrix(1:3, ncol = 1), widths = 1,  heights = c(5,5,6))
plotWAC (DE[, c("Time", "PITCH", "PITCH_IRS2", "CPITCH_GP")])
plotWAC (DE[, c("Time", "ROLL", "ROLL_IRS2", "CROLL_GP")])
op <- par (mar=c(5,4,1,1)+0.1)
plotWAC (DE[, c("Time", "THDG", "THDG_IRS2", "CTHDG_GP")])
...
The last set of components entering the measurement of wind consists of the measurements of the motion of the aircraft with respect to the Earth. These measurements must be combined with the measurement of relative wind to transform the measurements to an Earth-referenced measurement. Figure~\ref{fig:radome-plot-groundspeed} shows the east and north components of the groundspeed as measured by the IRU and GPS. They are close enough to lie almost on top of each other in this plot, but the red lines show the difference magnified by a factor of 100. They clearly show the Schuler oscillation that results from an IRU error in pitch, having magnitude of about 1--2~m\,s$^{-1}$. This error is discussed in the next section, and Section XXX discusses how the IRU measurements (having good short-term response but long-term drift) and the GPS measurements (having long-term accuracy but inferior short-term response) are combined in the measurement of wind. In addition to the Schuler oscillation, additional perturbations associated with turns result from the mixing of pitch, roll, and heading errors when the aircraft is banked.
...
absence of such a correction, PS\_GP should not be considered an alternate measurement of ambient pressure. The use of this measurement to determine an alternate measure of true airspeed will be discussed in Section 4.3.\marginpar{XXX}.
...
port on the Rosemouint 858 probe and the static ports on the side
...
is developed in Section \ref{sec:Calibrations}. On the basis of that
...
<<vw-gp-vs-wic, include=TRUE, fig.cap="Comparison of vertical wind calculated from the gust pod (WI\\_GP) to the conventional measurement of vertical wind (WIC).">>=
...
<<hw-gp-vs-std, include=TRUE, fig.cap="Comparison of vertical wind direction (top panel) and vertical wind speed (bottom panel) as determined from the gust pod and from the conventional radome-based system.">>=

op <- par (mar=c(2,4,1,1)+0.1)
layout(matrix(1:2, ncol = 1), widths = 1,  heights = c(5,6))
plotWAC(DE[, c("Time", "WDC", "WD_GP")], ylab="wind direction [deg.]")
op <- par (mar=c(5,4,1,1)+0.1)
plotWAC(DE[, c("Time", "WSC", "WS_GP")], ylab="wind speed [m/s]")
...
a comparison of the vertical wind calculated from the gust pod (WI\_GP) vs the conventional vertical wind (WIC). The standard deviation between the two measurements is about 0.3\,m/s.
This is a good illustration because this is a flight with large-amplitude waves and some
...
gust pod and the conventional wind-sensing system. In this case, so spikes are evident
in the measurements from the gust pod. These are the result of a problem with the C-MIGITS
measurement of heading, which exhibits noisy fluctuations as it moves through 180$^{\circ}$. 
These are a problem for the horizontal wind measurements, esp.~the east component. They
can be removed by spike-removal routines but are present in first-pass data files. Almost
all appear in turns where the measurements from the gust pod should not be used because errors arising
from misalignment of the gust-pod IRU with respect to the aircraft longitudinal axis 
causes mixing of the attitude angles in turns.

<<vw-gp-small-segment, include=TRUE, fig.cap="A 15-min segment from flight 16 of the DEEPWAVE project, showing the good agreement of the vertical wind measurements from the gust pod (WI\\_GP) and from the conventional wind-sensing system on the GV (WIC). The dashed orange line is a 1:1 reference line, and each blue dot represents a 1-s pair of corresponding measurements.">>=
...
\protect\protect\protect\caption{\label{fig:VarSpecWICandWIG}variance spectra for WIC (red line, and
...
angles in turns has not been developed or implemented.
...
which is not aligned with the longitudinal axis of the aircraft. The canisters
on the GV are oriented with axis pointing inward and downward relative to the
aircraft longitudinal axis, in order to align with normal flow at the pods. This
...
Kalman filter will use GPS measurements to correct such errors in the
...
course of the flight via the C-MIGITS Kalman filter.
\end{enumerate}
...
may be able to reduce these weaknesses, but that will require an
extensive investigation.\marginpar{XXX}
...
\subsection{The laser air-motion sensor (LAMS)\label{sub:LAMS-description}}

The LAMS did not function properly in DEEPWAVE. It will not be discussed
in detail in this report because it is still under development. The
characteristics and associated uncertainties in measured wind are discussed
by \cite{SpulerEtAl2011} and \cite{CooperEtAl2014}. The system consists of
one or several fiber-based laser beams focused ahead of the aircraft and
