...
<<LAMS-attack-plot, include=TRUE, fig.cap="Angle of attack determined by the standard system (AKRD) plotted vs.\ the angle of attack determined from the LAMS. The orange dashed line is the regression line for AKRD as a function of the LAMS angle-of-attack.">>=
...
The flight shown previously included a speed run from 20:32--20:42 UTC. Using the LAMS-determined angle of attack as the reference $\alpha^{*}$ in a fit of the form \eqref{eq:AOAsens1term} gave a regression slope of \Sexpr{round(cfLAMS[2], 2)}$^{\circ}$, 
