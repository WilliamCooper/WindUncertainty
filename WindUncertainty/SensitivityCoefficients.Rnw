...
$w_{p}=$GGVSPDB, and $V=$TASX. The radome measures the pressure
...
theory that one element of $\{x_{i}\}$ will be ADIFR\_GP/QC\_GP.
...
with a FL430 leg from 9:30 to 11:00 UTC; and an expanded section from
...
as in (\ref{eq:AOAstandard}) resulted in negligible improvement (0.0001$^{\circ}$)
...
for the larger dataset, for the speed runs alone, and using the \textquotedbl{}standard\textquotedbl{}
...
resulted in values 0.029$\pm$0.007$^{\circ}$ larger than those from
the speed-run coefficients and 0.025$\pm$0.045$^{\circ}$ larger
...
to those obtained using the fit to the speed-run dataset. For all
the points in the larger dataset, the respective differences in the
average angle-of-attack between the speed-run fit and the expanded-data-set
fit and between the speed-run fit and the standard fit (\ref{eq:AOAstandard})
were 0.029$\pm$0.007$^{\circ}$ and 0.025$\pm$0.045$^{\circ}$.
This indicates that any of these formulas would give about the same
results and that one measure of uncertainty in the results from applying
...
and $\Delta p_{\beta}=$BDIFR\_GP. The resulting fit for the radome
is listed below:
...
The measurements ADIFR\_GP and BDIFR\_GP, normalized by QC\_GP, are
...
from the gust-pod static and dynamic pressure, and $\Delta P_{\alpha,g}$=ADIFR\_GP
...
result for $G^{*}\sim q/p$, the true airspeed can then be obtained
...
However, this is larger than the estimated uncertainty in TASX (about
0.3\,m/s), so wind calculations will be better if based on the conventional
measurement of TASX unless it is desirable to calculate an estimate
of true airspeed based entirely on the gust pod and independent of
the standard measurement, or in case there is a problem with the standard
measurement. 
...
\subsection{Summary of sensitivity coefficients}


\subsubsection{Angle of Attack}

Radome: Eq.~(\ref{eq:AOArecommended}), coefficients $c_{0,1}$ =\{\Sexpr{round(cfr[1],3)},
\Sexpr{round(cfr[2],3)}\}.\\
Gust pod: Eq.~(\ref{eq:AOAGfunction}), coefficients\{$b_{i=0,3}$\}
= \{\Sexpr{round(cf3_GP[1],3)}, \Sexpr{format(round(cf3_GP[2],3),nsmall=3)},
\Sexpr{round(cf3_GP[3],3)}, \Sexpr{round(cf3_GP[4],3)}\}.


\subsubsection{Sideslip Angle}
\begin{hangparagraphs}
Radome and gust pod: Eq.~(\ref{eq:betaFunctionForm}), coefficients
\{$e_{0}$, $e_{1}$\} = \\
\qquad{} \{\Sexpr{format(round(cfs[1],3),nsmall=3)}, \Sexpr{round(cfs[2],3)}\}
for the radome,\\
...
\Sexpr{cf4[6]}\}.\end{hangparagraphs}
