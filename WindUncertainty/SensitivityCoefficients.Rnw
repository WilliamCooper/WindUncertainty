...
without correction for pitch errors and negligible with such correction (because
that is required by the method of determining sensitivity coefficients), in both
...
and the residual standard error was \Sexpr{round (summary(sfm)$sigma, 2)},
