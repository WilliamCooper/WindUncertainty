...
\Sexpr{format(round(cfr[1],3), nsmall=3)} and \Sexpr{round(cfr[2],3)}.%
The fit accounted for \Sexpr{round(100*summary(fmy)$r.squared, 1)}\% of the variance and had a residual standard error of \Sexpr{round(summary(fmy)$sigma, 2)}$^{\circ}$, so it represents the speed-run data reasonably well.%
%%%% skip
In Fig.~\ref{fig:AOA-fit-and-plot},
...
and a standard error of \Sexpr{round(summary(fm)$sigma,2)}%
...
as in (\ref{eq:AOAstandard}) resulted in negligible improvement (0.001$^{\circ}$) in the standard error, and introduction of a set of eight possible
...
equations than (\ref{eq:AOArecommended}) do not appear to be needed.\footnote{This simpler representation applies to the normal research flight
...
a value for the first coefficient of \Sexpr{round(summary(fmconst)$coefficients[1], 3)} and a standard error negligibly different from that for the two-coefficient
...
(\ref{eq:AOArecommended}) with these values for the coefficients: \{$c_{0},\, c_{1}$\} = \{\Sexpr{round(cf31,3)}, \Sexpr{round(cf32,3)}\}.
...
larger than those from the standard coefficients, so using any of these formulas
...
= \{\Sexpr{round(cf3_GP[1],3)}, \Sexpr{format(round(cf3_GP[2],3),nsmall=3)}, \Sexpr{round(cf3_GP[3],3)}, \Sexpr{format(round(cf3_GP[4],3),nsmall=3)}\}%
...
were \{e\} = \{\Sexpr{format (round(cfs[1], 3), nsmall=3)}, \Sexpr{round (cfs[2], 3)}\},%
...
\{$e_{0}$, $e_{1}$\} = \\%
\qquad{} \{\Sexpr{round(cfs[1],3)}, \Sexpr{round(cfs[2],3)}\} for the radome,%
\footnote{However, see the revision of the first coefficient that follows in Section \ref{sec:HWind}. The value listed in the concluding summary of that section, in Section \ref{sub:HWsummary}, should supersede the value listed here.} \\%
\qquad{} \{\Sexpr{round(cfs_GP[1],3)}, \Sexpr{round(cfs_GP[2],3)}\} for the gust pod.%
\end{hangparagraphs}
