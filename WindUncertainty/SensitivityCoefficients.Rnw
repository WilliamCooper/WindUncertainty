...
# this is repeated from SchuilerSection, for independent compilation to PDF
...
# --now in parent file. For ReloadData = FALSE, retrieve from previously saved data
...
if (ReloadData) {
  ...
  flight = "rf03"
  fname = sprintf("%s%s/%s%s.nc", DataDirectory (), Project, Project, Flight)
  D6 <- getNetCDF (fname, VarNames)
  D6["PITCHC"] <- D6$PITCH - CorrectPitch (D6)
  D6 <- D6[D6$GGALT/0.3048 > 35000, ]
...
capnF1b <- "The angle of attack determined from the simple fit to the pressure ratio from the radome, as a function of the angle-of-attack predicted for zero vertical wind from the formula in the text (blue shapes), and the corresponding distribution that would result from using the standard calibration (green shapes). The shapes show the distributions for measurements in centered bins at (1/3)-deg increments in the predicted angle-of-attack. The dashed orange line is the best fit to the speed-run measurements."

@

This fit gave coefficients $c_{0}^{*}$ and $c_{1}^{*}$ equal to
\Sexpr{format(round(summary(fmy)$coef[1],3), nsmall=3)} and \Sexpr{round(summary(fmy)$coef[2],3)}.
The fit accounted for \Sexpr{round(100*summary(fmy)$r.squared, 1)}\%
of the variance and had a residual standard error of \Sexpr{round(summary(fmy)$sigma, 2)}$^{\circ}$,
so it represented the speed runs well. In Fig.~\ref{fig:AOA-fit-and-plot},
the thin grey line under the thick orange dashed line represents the
...
the calibration, a series of fits like that leading to the above formula
and coefficients were performed after shifting the measurement of
...
# beware: can't use this with ReloadData==TRUE, r4 missing
...
measured sensitivity as in (\ref{eq:AOAsens1term}). The expanded
...
Data$A3 <- cf3[1]+cf3[2]*Data$AQR     # "larger" dataset
...
\end{equation}
led to best-fit coefficients \{$c_{0},\, c_{1}$\} = \{\Sexpr{round(cf31,3)},
\Sexpr{round(cf32,3)}\} and a standard error of \Sexpr{round(summary(fm)$sigma,2)}
...
slope parameter to the value obtained from the speed-run data, \Sexpr{round(cfr[2], 3)},
...
a value for the first coefficient of \Sexpr{round(summary(fmconst)$coefficients[1], 3)}
...
\{$c_{0},\, c_{1}$\} = \{\Sexpr{round(cf31,3)}, \Sexpr{round(cf32,3)}\}.


Evaluation of all the points in the larger dataset using three fits,
that from the larger dataset, from the speed runs alone, 
and using the \textquotedbl{}standard\textquotedbl{}
fit discussed above, gave only very small differences in the results:
the angle-of-attack evaluated from the larger-dataset coefficients
resulted in values 
\Sexpr{format(round(mean(Data$A3-Data$A1, na.rm=TRUE),3), nsmall=3)}%
$\pm\Sexpr{format(round(sd(Data$A3-Data$A1, na.rm=TRUE),3), nsmall=3)}%
^{\circ}$ larger than those from
the speed-run coefficients and 
\Sexpr{round(mean(Data$A3-Data$A3std, na.rm=TRUE),3)}%
$\pm\Sexpr{round(sd(Data$A3-Data$A3std, na.rm=TRUE),3)}^{\circ}$ larger
than those from the standard coefficients, so using any of these formulas
would give about the same results.  Still another determination of the
...
ysz = 0.30
# g <- g + geom_rect(aes(xmin=3.5-xsz/2.,xmax=3.5+xsz/2.,ymin=0.6-ysz/2.,ymax=0.6+ysz/2.), fill="lightyellow", colour="blue")
# g <- g + annotate("text", label="larger-data-set residual: 0.13 deg.", x=3.5,y=0.6,size=6, colour="red")
g <- g + xlab (expression(paste("Reference AOA [", degree, "]")))
g <- g + ylab (expression(paste("AOA from fit [",degree,"]"))) + ylim(0.,6.)
g <- g + geom_line(aes(y=AOAREF), colour="darkorange", lwd=2, linetype=2)
g <- g + theme_WAC ()
...
The standard errors in the coefficients for the larger dataset are
respectively \Sexpr{round(summary(fm)$coefficients[1,2],4)} and \Sexpr{round(summary(fm)$coefficients[2,2],3)},
so the coefficients are tightly constrained by the fit. Surprisingly,
the correlation between error terms was very high and positive, increasing
the combined effects of these errors in any calculation of angle-of-attack.
The larger dataset provided results consistent with those from the
speed runs alone but contained 17,721 measurements vs.~only 2,019
for the speed-run dataset, so the larger set provides an opportunity
...
randomly into 100 subsets and the fit coefficients calculated for
...
and 0.88. Repeating this for different subset sizes $N$ indicated
...
from regions with true vertical wind. The subsetting also revealed
that the individual coefficients were highly correlated, with correlation
coefficient 0.99. The effect of this is the two error terms add almost
linearly, not in quadrature, in their effect on the final uncertainty
in angle of attack.

The effect of these errors on the resulting angle of attack, for this
dataset, was evaluated by applying randomly generated Gaussian fluctuations
to the coefficients and calculating the net effect on the parameterized
representation. The resulting standard error in angle-of-attack was
0.001$^{\circ}$. However, because the speed-run values and standard
values gave deviations relative to this calibration of about 0.03$^{\circ}$,
that may be a better estimate of the uncertainty in angle of attack
from possible bias, with the smaller standard error representing the
random component of that uncertainty. The net effect on vertical wind
produced by 0.03$^{\circ}$ uncertainty in the sensitivity coefficients
for angle-of-attack is therefore estimated to be about 0.1~m/s from
...
if (ReloadData) {
  ...
  for (j in c(1:5,8:14,16:26)) {     	
    fname = sprintf("%s%s/%srf%02d.nc", Directory,Project,Project,j)
    ...
    DataC <- merge (DataC, DW[Valid, ], all=TRUE)
  ...
  save (DataC, file='~/RStudio/DEEPWAVE/WindUncertainty/AOAdata.Rdata')
} else {
  load (file='~/RStudio/DEEPWAVE/WindUncertainty/AOAdata.Rdata')
}
...
\subsubsection{Application to the gust pod}
...
= \{\Sexpr{round(cf3_GP[1],3)}, \Sexpr{format(round(cf3_GP[2],3),nsmall=3)},
\Sexpr{round(cf3_GP[3],3)}, \Sexpr{format(round(cf3_GP[4],3),nsmall=3)}\}
and the square of the correlation was \Sexpr{format(round(summary(fm_GP)$r.squared,3),nsmall=2)}
with residual scatter (residual standard error) of \Sexpr{format(round(summary(fm_GP)$sigma,3),nsmall=2)}.
...
if (ReloadData) {
  ...
  save (DataC, file="~/RStudio/DEEPWAVE/WindUncertainty/AOAGPdata.Rdata")
} else {
  load (file='~/RStudio/DEEPWAVE/WindUncertainty/AOAGPdata.Rdata')
}
DataS <- DataC
...
@
These coefficients \{\Sexpr{round(cfGPallflt[1], 3)}, \Sexpr{format(round(cfGPallflt[2], 3), nsmall=3)}, \Sexpr{round(cfGPallflt[3], 3)}, \Sexpr{round(cfGPallflt[4], 3)}\} may be preferable to the coefficients obtained above if it is desirable to minimize the all-project mean vertical wind for the gust-pod measurements.
...
4:31:00--4:33:30, and 5:26:40--5:29:30 UTC. Because the yaw maneuvers
on rf11 (10:25:00 to 10:30:00) were flown in the old way, they were
...
\subsubsection{Application to the radome-based and gust-pod systems}
...
<<sideslip-fits, include=TRUE, fig.cap="The sideslip attack determined from the fit, as a function of the reference angle provided by Equation (\\ref{eq:beta-equation}), for the combination of data from all three yaw maneuvers listed in the text. The gray area underlying the dashed orange line denotes the standard-uncertainty range for the fit.", fig.height=4.5>>=
...
The best-fit coefficients were \{e\} = \{\Sexpr{format (round(cfs[1], 3), nsmall=3)},
\Sexpr{round (cfs[2], 3)}\}, the squared correlation was \Sexpr{round (summary(sfm)$r.squared, 2)}
and the residual standard error was \Sexpr{round (summary(sfm)$sigma, 3)},
as listed above. The plotted measurements and this fit are shown in
Fig.~\ref{fig:sideslip-fits}. The near-zero first coefficient is
a forced result from using calculated wind components in (\ref{eq:beta-equation}).
...
slightly smaller than the value determined from this calibration.
...
The squared correlation of this fit was \Sexpr{round (summary(sfm_GP)$r.squared, 2)}
and the residual standard error was \Sexpr{round (summary(sfm_GP)$sigma, 3)}$^{\circ}$.
The best-fit coefficients were \{\Sexpr{format (round(cfs_GP[1], 3), nsmall=3)},
\Sexpr{round (cfs_GP[2], 3)}\}. The difference in first coefficient
...
for sideslip. An offset in sideslip, arising from an erroneous first
coefficient $e_{0}$ in the calibration, would produce an offset in
the lateral component of the measured wind, and that erroneous lateral
component would enter the calibration equation to again give a first
coefficient near zero. An offset therefore must be determined in another
way. Two candidates for finding the offset are reverse-heading maneuvers,
during which the horizontal wind component lateral to the aircraft
should remain constant but change sign, or circle maneuvers, during
which an offset in sideslip will appear as a sinusoidally varying
perturbation to the wind. Later in this report, a set of circles,
flown with constant roll, will be used in Section \ref{sub:Analysis-of-circle}
to obtain a better estimate of the sideslip offset. .\vfill\eject


\subsection{True Airspeed from the Gust Pod}
...
<<plot-qp-fit, include=TRUE, fig.cap="The estimate of $q/p$ obtained from the right side of (\\ref{eq:TASfit}), determined entirely from measurements provided by the gust pod, plotted against the standard measurement obtained from QCXC/PSXC. The coefficients are listed in the text.", fig.height=4.5>>=
...
representation of TASX, with a standard error of about \Sexpr{round(taserr,1)}\,m/s.
...
plot(DataV$TASX, DataV$Vg, xlab="TASX [m/s]", ylab="TAS, gust pod [m/s]", xlim=c(150.,250.), ylim=c(150.,250.),pch=16, cex=0.7, col='blue') 
...
Radome: Eq.~(\ref{eq:AOArecommended}), coefficients $c_{0,1}$ =\{\Sexpr{round(cf3[1],3)},
\Sexpr{format(round(cf3[2],3), nsmall=3)}\}.\footnote{See also the alternative coefficients \{\Sexpr{round(cfallflt[1], 3)}, \Sexpr{round(cfallflt[2], 3)}\} discussed on page \pageref{coefficients-aoa-all-flt}.}\\
Gust pod: Eq.~(\ref{eq:AOAGfunction}), coefficients\{$b_{i=0,3}$\}
= \{\Sexpr{round(cf3_GP[1],3)}, \Sexpr{format(round(cf3_GP[2],3),nsmall=3)},
\Sexpr{round(cf3_GP[3],3)}, \Sexpr{format(round(cf3_GP[4],3), nsmall=3)}\}\footnote{See also the alternative coefficients \{\Sexpr{round(cfGPallflt[1], 3)}, \Sexpr{format(round(cfGPallflt[2], 3), nsmall=3)}, \Sexpr{round(cfGPallflt[3], 3)}, \Sexpr{round(cfGPallflt[4], 3)}\} discussed on page \pageref{aoa-coefficients-GP-all-flt}.}
...
\{$e_{0}$, $e_{1}$\} = \\
\qquad{} \{\Sexpr{round(cfs[1],3)}, \Sexpr{round(cfs[2],3)}\}
for the radome,%
...
\\
\qquad{} \{\Sexpr{round(cfs_GP[1],3)}, \Sexpr{round(cfs_GP[2],3)}\}
for the gust pod.
\end{hangparagraphs}
...
\{$b_{i=0,7}$\} = \\
\qquad{} \{\Sexpr{cf4[1]}, \Sexpr{cf4[2]},\inputencoding{latin1}{
}\inputencoding{latin9}\Sexpr{cf4[3]}, \Sexpr{cf4[4]}, \Sexpr{cf4[5]},
\Sexpr{cf4[6]}, \Sexpr{cf4[7]}, \Sexpr{cf4[8]}\}.\end{hangparagraphs}
