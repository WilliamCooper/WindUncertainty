...
if (ReloadData) {
  ...
  D2x["PITCHC"] <- D2x$PITCH - CorrectPitch (D2x)[, 1]
...
if (ReloadData) {
  ...
  D3x["PITCHC"] <- D3x$PITCH - CorrectPitch (D3x)[, 1]
...
if (ReloadData) {
  ...
  D4x["PITCHC"] <- D4x$PITCH - CorrectPitch (D4x)[, 1]
...
if (ReloadData) {
  ...
  D5x["PITCHC"] <- D5x$PITCH - CorrectPitch (D5x)[, 1]
...
if (ReloadData) {
  ...
  D6x["PITCHC"] <- D6x$PITCH - CorrectPitch (D6x)[, 1]
...
accounted for \Sexpr{round(100*summary(fmy)$r.squared, 1)}\% of the variance and had a residual standard deviation\footnote{The R routine 'lm' produces a quantity labeled "residual standard error" that represents the standard deviation between the fit results and the measurements, but this will be described here as the residual standard deviation to clarify that this is the standard deviation of individual points, not of the mean.}\index{residual standard deviation} of \Sexpr{round(summary(fmy)$sigma, 2)}$^{\circ}$, so it represents the speed-run data reasonably well.\footnote{A residual standard deviation of this magnitude corresponds to the measured fluctuations
...
and a residual standard deviation of \Sexpr{round(summary(fm)$sigma,2)}$^{\circ}$  
...
with residual scatter (residual standard deviation) of \Sexpr{format(round(summary(fm_GP)$sigma,3),nsmall=2)}$^{\circ}$. % 
...
and the residual standard deviation was \Sexpr{round (summary(sfm)$sigma, 2)}$^{\circ}$,  
...
The squared correlation of this fit was \Sexpr{round (summary(sfm_GP)$r.squared, 2)} and the residual standard deviation was \Sexpr{round (summary(sfm_GP)$sigma, 3)}$^{\circ}$. The best-fit coefficients were \{\Sexpr{format (round(cfs_GP[1], 3), nsmall=3)}$^{\circ}$, 
...
<<reload-LAMS-3D-data, include=FALSE>>=

DataFrameFileL <- "DataFrames/DataFrameL.Rdata"
## data previously saved in ComponentsOfWindSensingSystems.Rnw
DataSave <- Data
...
<<LAMS-transform, include=FALSE>>=

# DataL$BEAM1speed <- DataL$Beam1_LAMS
# DataL$BEAM2speed <- DataL$Beam2_LAMS
# DataL$BEAM3speed <- DataL$Beam3_LAMS

DDX <- DataL
DataL$BEAM1speed <- zoo::na.approx (as.vector(DataL$BEAM1speed), maxgap=1000, na.rm = FALSE)
DataL$BEAM2speed <- zoo::na.approx (as.vector(DataL$BEAM2speed), maxgap=1000, na.rm = FALSE)
DataL$BEAM3speed <- zoo::na.approx (as.vector(DataL$BEAM3speed), maxgap=1000, na.rm = FALSE)

## try to fix CTHDG_LAMS bad points in transition through 180:
for (i in 2:(nrow(DataL)-1)) {
  if (is.na(DataL$CTHDG_LAMS[i]) || is.na(DataL$CTHDG_LAMS[i-1]) || is.na(DataL$CTHDG_LAMS[i+1])) {next}
  if (abs(DataL$CTHDG[i-1]-DataL$CTHDG_LAMS[i+1]) > 10.) {next}
  if ((DataL$CTHDG_LAMS[i-1] < 180.) && (DataL$CTHDG_LAMS[i+1] > 180)) {DataL$CTHDG_LAMS[i] <- NA}
  if ((DataL$CTHDG_LAMS[i-1] > 180.) && (DataL$CTHDG_LAMS[i+1] < 180)) {DataL$CTHDG_LAMS[i] <- NA}
}
DataL$CTHDG_LAMS <- zoo::na.approx (as.vector(DataL$CTHDG_LAMS), maxgap=1000, na.rm = FALSE)
DataL <- transferAttributes (DataL, DDX)
...
A = c(DataL$BEAM1speed, DataL$BEAM2speed, DataL$BEAM3speed)
dim(A) <- c(nrow(DataL), 3)
RW = t (Si %*% t(A))    # gives u, v, w components, RW[,1] is u
# D <- data.frame("Time"=DataL$Time)
D <- DataL
D$TASX <- sqrt(RW[,1]^2 + RW[,2]^2 + RW[,3]^2)
D$ATTACK <- atan (RW[, 3] / RW[, 1]) * 180 / pi
D$SSLIP <-  atan (RW[, 2] / RW[, 1]) * 180 / pi
D$GGVSPD <- DataL$CVSPD_LAMS
D$VEW <- DataL$CVEW_LAMS
D$VNS <- DataL$CVNS_LAMS
D$THDG <- DataL$CTHDG_LAMS
D$ROLL <- DataL$CROLL_LAMS
D$PITCH <- DataL$CPITCH_LAMS +0.15
rg <- setRange (DataL$Time, 193000,205000)
D2 <- D[rg, ]
D <- transferAttributes (D2, DataL)
Data2 <- DataL[rg, ]
DataL <- transferAttributes (Data2, DataL)
DW <- WindProcessor (data=D)
DataL$WD_LAMS <- DW$WDN
attr(DataL$WD_LAMS, "units") <- "degrees"
DataL$WS_LAMS <- DW$WSN
attr(DataL$WS_LAMS, "units") <- "m/s"
DataL$WI_LAMS <- DW$WIN
attr(DataL$WI_LAMS, "units") <- "m/s"
DataL$ATTACK_L <- D$ATTACK
attr(DataL$ATTACK_L, "units") <- "degrees"
DataL$SSLIP_L <- D$SSLIP
attr(DataL$SSLIP_L, "units") <- "degrees"
DataL$TASL <- D$TASX
attr(DataL$TASL, "units") <- "m/s"

@


<<LAMS-attack-plot, include=TRUE, fig.scap="Standard angle of attack vs.\ the angle of attack determined from the LAMS.", fig.cap="Angle of attack determined by the standard system (AKRD) plotted vs.\ the angle of attack determined from the LAMS. The orange dashed line is the regression line for AKRD as a function of the LAMS angle-of-attack.">>=

rb <- setRange (DW$Time, 203200,204200)
op <- par (mar=c(5,4,1,1)+0.1)
DW$ATTACK[DW$ATTACK < -2] <- NA
DW$ATTACK[DW$ATTACK > 10] <- NA
DW$ATTACK[DW$TASX < 130] <- NA
fLAMS <- lm(ATTACK~I(ADIFR/QCF), data=DW)
cfLAMS <- coef (fLAMS)
#plot(DW$ATTACK[rb], DW$AKRD[rb], type='p', pch=20, col='blue', ylab="Standard AKRD [deg.]", xlab="LAMS-based angle of attack [deg.]")
g <- ggplot (data=DW[rb, ], aes(x=ATTACK))
g <- g + geom_point (aes(y=AKRD), pch=20, col='blue', na.rm=TRUE) 
# g <- g + geom_line (aes (x=c(-2.,2.), y=c(-2.,2.)), data=NULL, col='darkorange', lwd=1.3, lty=2) 
  g <- g + ylab("Standard AKRD [deg.]") +
  xlab("LAMS-based angle of attack [deg.]") + theme_WAC ()
...
x <- c(-3.,2.)
y <- cf[1]+cf[2]*x
DAA <- data.frame ("x"=x, "y"=y)
g <- g + geom_line (aes(x=x, y=y), data=DAA, col='darkorange', lwd=1.3, lty=2)
print (g)

@

The flight shown previously included a speed run from 20:32--20:42 UTC. Figure~\ref{fig:LAMS-attack-plot}\sindex[var]{AKRD>@AKRD} shows the measurements of angle of attack determined from the standard calibration plotted vs.\ those determined using the LAMS. Using the LAMS-determined angle of attack as the reference $\alpha^{*}$ in a fit of the form \eqref{eq:AOAsens1term} gave a regression slope of \Sexpr{round(cfLAMS[2], 2)}$^{\circ}$, 
...
fyLAMS <- lm (DW$SSLIP[rd]~I(DW$BDIFR[rd]/DW$QCF[rd]))
cfyLAMS <- coef(fyLAMS)
## SummarizeFit(fd)
...
measurements in \eqref{eq:betaFunctionForm}. The result was a second coefficient of \Sexpr{round(cfyLAMS[2], 2)}$^{\circ}$, which differs by only about 4\% from the value of 22.32$^{\circ}$ obtained from the sideslip maneuvers. The sideslip angle in normal flight seldom exceeds 1$^{\circ}$, so
