...
# change to fit for groundspeed magnitude:
# csq <- function (x, D) {
#   hdg <- (D$THDG + x[4]) * pi / 180.
#   ggs <- (D$GGVEWB**2 + D$GGVNSB**2)**0.5
# #   ggs <- (D$VEW**2 + D$VNS**2)**0.5
#   ags <- ((x[2] + x[1]*sin(hdg))**2 + (x[3] + x[1]*cos(hdg))**2)**0.5
#   chisq <- sum((ggs-ags)**2)
# }
...
for (n in 1:29) {
 ...
 #print(sprintf(" wind estimate: %.1f / %.1f", atan2(wx,wy)*180/pi+180, sqrt(wx**2+wy**2)))
  ...
  rms[n] <- (A$minimum/(2*length(D$THDG)))**0.5
  if (rms[n] < rmsmin) {
    rmsmin <- rms[n]
    bestFit <- A$estimate
    nbest <- n
...
plot(1:29, rms[1:29], type='l', col='blue', lwd=2, xlab="lag, number of 25-Hz samples", ylab="RMS(lag) [m/s]")
...
# best-fit wind:
bestWD <- atan2(bestFit[2], bestFit[3]) / Cradeg + 180.
if (bestWD > 360) {bestWD <- bestWD - 360}
bestWS <- sqrt(bestFit[2]**2 + bestFit[3]**2)
print(sprintf(" fit results: best offset %d 25-Hz samples, V %.1f (vs measured mean %.1f), H offset %.1f, wind %.1f / %.1f", nbest,bestFit[1], mean(D$TASX),bestFit[4], bestWD, bestWS))
...
# ags <- ((x[2] + x[1]*sin(hdg))**2 + (x[3] + x[1]*cos(hdg))**2)**0.5
# dgs <- ggs - ags
plotWAC(D$Time, dvx, ylab="deviation, EW [blue] or NS [green]")
lineWAC(D$Time, dvy, col='darkgreen')
lineWAC(D$Time, D$THDG/180-1, col='darkorange')
Angle <- (D$THDG-bestWD)%%360.
plot(Angle, dvx, pch=16, col='blue')
points(Angle, dvy, pch=16, col='darkgreen')
lines(Angle, -sin(2.*Angle*pi/180), col='red')
lines(c(0.,0.), c(-5.,5.), col='green')
lines(c(180.,180.), c(-5.,5.), col='green')
lines(c(360.,360.), c(-5.,5.), col='green')
...
D <- Dsave[r,]
WX <- sin(D$WDC*pi/180) * D$WSC
WY <- cos(D$WDC*pi/180) * D$WSC
D["WX"] <- WX
D["WY"] <- WY
csq2 <- function (x, D) {
  ...
  TASX <- D$TASX + x[1]
  dux <- D$WX + x[1] * sinpsi + TASX*x[2]*cospsi-x[3]
  duy <- D$WY + x[1] * cospsi - TASX*x[2]*sinpsi-x[4]
...
bestWD <- (atan2(A2$estimate[3], A2$estimate[4]) / Cradeg)%%360
bestWS <- sqrt(A2$estimate[3]**2 + A2$estimate[4]**2)
print(sprintf(" Results: dV=%5.2f, dH=%5.2f, WD=%.1f, WS=%.1f, rms=%.2f", 
...
Ang <- (D$THDG - 223.1) %% 360
WS <- D$WSX
plot (Ang, WS, xlim=c(0,360), pch=19, col='white', xlab="Flight direction minus wind direction", ylab="Wind Speed [m/s]")
rs <- (D$ROLL < -15.)
points (Ang[rs], WS[rs], pch=19, col='blue')
rs <- (D$ROLL > 15.)
points (Ang[rs], WS[rs], pch=19, col='darkgreen')
...
WS <- Dsave$WSX[r1]
Ang <- (Dsave$THDG[r1] - 223.1) %% 360
fm <- lm (WS~sin(Ang*pi/180.)+cos(Ang*pi/180.))
summary(fm)
cf <- coefficients(fm)
lines(Ang, cf[1]+cf[2]*sin(Ang*pi/180.)+cf[3]*cos(Ang*pi/180.), col='darkorange', lwd=4)
WS <- Dsave$WSX[r2]
Ang <- (Dsave$THDG[r2] - 223.1) %% 360
fm <- lm (WS~sin(Ang*pi/180.)+cos(Ang*pi/180.))
summary(fm)
cf <- coefficients(fm)
lines(Ang, cf[1]+cf[2]*sin(Ang*pi/180.)+cf[3]*cos(Ang*pi/180.), col='darkorange', lwd=4)
print(sprintf("mean direction [WDC. WDX] C %.1f WDX %.1f mean speed %.1f %.1f", mean(D$WDC,na.rm=TRUE), mean(D$WDX, na.rm=TRUE), mean(D$WSC, na.rm=TRUE), mean(D$WSX, na.rm=TRUE)))
