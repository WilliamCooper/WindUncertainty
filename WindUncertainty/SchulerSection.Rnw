...
<<plot-illustrating-pitch-coupling, fig.cap="Illustration of how a pitch error leads to a varying false horizontal acceleration: (left) A pitch error of magnitude $\\delta\\theta$ leads to a false measurement of horizontal acceleration of magnitude $g\\delta\\theta$. The blue line represents the true level plane tangent to the surface of the Earth and $g$ the magnitude and direction of the gravitational acceleration. (right) An error in the measurement of position, denoted here as an error in latitude $\\delta\\lambda$, leads to an error in the direction of the vertical axis and so to a changed error in horizontal acceleration.", fig.width=2.6, fig.height=3.1, include=TRUE, fig.show='hold'>>=
...
<<v-errors-straight-leg, fig.lp="fig:",fig.cap="Errors in the NS and EW components of ground speed as determined by comparison to GPS, for DEEPWAVE flight ff02, a ferry flight starting on 1 June 2014 and traveling from Hawaii to Pago-Pago.", include=TRUE>>=
...
sdc <- sd (CorrectPitch(D)[Valid], na.rm=TRUE)
