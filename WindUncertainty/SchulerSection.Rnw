...
Some possible tests that could lead to some confidence that the correction
...
\\
The following is a tabulation of 5 instances where the flight track
...
it would be expected that the vertical wind would be the same. A 
the pitch error, if caused by gyro misalignment, would reverse
sign between these legs, increasing the measured vertical wind on one leg while 
...
pitch-correction algorithm did reduce this average to about half the
uncorrected value, as shown in Fig.\ \ref{fig:reverse-course-w-comparison},
so the pitch correction kept the averages quite small and did appear to 
produce some improvement as expected.
...
measurements above 25,000\,ft and with true airspeed above 130\,m/s 
were included to emphasize normal
...
flight devoted to calibration will little upper-level flight and
frequent turns including circles.

<<mean-vertical-wind-by-flight, include=TRUE>>=
...
for (j in 1:26) {
    ...
    print (sprintf ("flight %d mean vertical wind %.02f, corrected %.02f", 
           j, mvw[j], mvwc[j]))
}
rv <- c(1:5,8:14,16:26)
...
The data restriction applied was that the true airspeed be above 130\,m/s and
...
The mean value of the vertical wind, for all flights combined, was $-0.08$\,m/s
for the uncorrected measurements and $-0.02$\,m/s for the corrected measurements,
so both are within expected tolerances but the corrected measurements are 
closer to zero for the full project. The pitch correction has little effect on
