...
d(GGVEW)/dt\\
d(GGVNS)/dt
\end{array}\right]\_\_\_.\label{eq:acceleration-from-GPS}
\end{equation}
...
following standard sources; see equation (55). Here the inverse transformation
...
D$dang <- (D$ang1-D$ang2)
t <- D$dang > 180
D$dang[t] <- D$dang[t] - 360
t <- D$dang < -180
D$dang[t] <- D$dang[t] + 360
D$dang <- signal::sgolayfilt (D$dang, 3, 60*data.rate+1, m=0)
plotWAC(D$Time, D$dang, ylim=c(-2,2))
r <- setRange(D$Time, 33800,35500)
plotWAC(D$Time[r], D$dang[r], ylim=c(-2,2))
t <- (D$LACCX^2+D$LACCY^2) < 1
D$dang[t] <- NA
D$DAX <- D$vedot - D$LACCX
D$DAY <- D$vndot - D$LACCY
D$DPSI <- (D$LACCY*D$DAX - D$LACCX*D$DAY) / (D$LACCX^2 + D$LACCY^2) / Cradeg
D$DPSIS <- signal::sgolayfilt (D$DPSI, 3, 60*data.rate+1, m=0)
VP <- c("Time", "DPSI", "DPSIS")
plotWAC (D[, VP], ylim=c(-2,2))
plotWAC (D[r, VP], lwd=c(2,4), lty=c(1,2), ylim=c(-2,2))
D$ACC <- sqrt(D$LACCX^2+D$LACCY^2)
Valid <- D$TASX > 130 & D$ACC > 1
DD <- D
DD[!Valid,] <- NA
plotWAC(DD[, VP], ylim=c(-2,2), lwd=c(1,3), col=c('blue', 'red'))
