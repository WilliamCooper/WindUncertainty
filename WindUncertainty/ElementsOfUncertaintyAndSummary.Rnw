...
The result of adding the elemental sources of uncertainty\index{uncertainty!vertical wind}\index{wind!vertical!uncertainty} in quadrature is an estimated bias uncertainty of %0.10
\Sexpr{format(round(sqrt(SQ1d), 2), nsmall=2)}~m\,s$^{-1}$ and an estimated random uncertainty of %0.07
\Sexpr{round(sqrt(SQ1e), 2)}~m\,s$^{-1}$. This estimate applies to measurements without pitch correction,\index{pitch!correction} and for such measurements the contribution from uncertainty in pitch is the dominant source of uncertainty. With the correction, the bias estimate decreases to 0.06\,m\,s$^{-1}$, so the pitch correction reduces the uncertainty significantly. 
