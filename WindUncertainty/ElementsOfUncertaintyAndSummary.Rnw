...
\midrule 
2 & AKRD coefficients & 0.03$^{\circ}$ & 0.001$^{\circ}$ & 0.12 & 0.04\tabularnewline
\midrule 
3 & BDIFR transducer  & 0.07~hPa  & 0.002~hPa  & -- & --\tabularnewline
\midrule 
4 & QCF transducer  & 0.34~hPa  & 0.01~hPa  & <0.02 & 0.001\tabularnewline
\midrule 
5 & pitch  & 0.02$^{\circ}$ & 0.02$^{\circ}$  & 0.08 & 0.08\tabularnewline
\midrule 
...
collected be zero. There is no independent way to check this except by 
comparing results from different regions as done in this report. That
remains a major weakness in calibration and leads to the fairly large estimated
bias in Table~\ref{tab:Elemental-w}. This may well be an over-estimate, especially
if large datasets are used (as in Section~sub:radome-sensitivity, where coefficients
were based on a large set of flights from DEEPWAVE). Those coefficients produced
a project-mean vertical wind of zero and so are likely less biased than indicated
by the entry in this table.
...
in Section~\ref{sec:Schuler} and \ref{subsub:prelim}. The partitioning
...
have the character of a random-error component, while for intervals
small compared to this the error will have the character of a bias. 
The estimate of random error tabulated
...
220\,m\,s$^{-1}$, is about 0.08\,m\,s$^{-1}$. The uncertainty in pitch
is the leading contributor to the standard uncertainty in vertical wind
and is also a significant contributor to the overall estimate of bias.
...
is a bias estimate of 0.15\,m\,s and a random-uncertainty estimate
of 0.09\,m\,s$^{-1}$. The dominant contributions are those from pitch and
...
reasonable to combine then in quadrature to obtain composite estimates.
...
result in lowered uncertainty limits of about 0.13\,m/s (bias) and 0.06\,m/s 
(standard uncertainty). Because the real bias arising from the calibration of
angle of attack may be smaller than this, it may be worthwhile to include
this correction in processing.
...
a full Kalman-filter correction to the measurements, which removes the need
...
\midrule 
2 & SSRD coefficients & 0.03$^{\circ}$ & 0.001$^{\circ}$ & (0.12,~--) & (0.04,~--)\tabularnewline
\midrule 
3 & ADIFR transducer  & 0.07~hPa  & 0.002~hPa  & -- & --\tabularnewline
\midrule 
4 & QCF transducer  & 0.34~hPa  & 0.01~hPa  & (<0.02,~0.3) & (0.001,~0.1)\tabularnewline
\midrule 
4 & heading & 0.09$^{\circ}$ & 0.02$^{\circ}$ &(~0.4, --) & (0.08,~--)\tabularnewline
\midrule
5 & pitch  & 0.02$^{\circ}$ & 0.02$^{\circ}$  & -- & --\tabularnewline
\midrule 
6 & horiz. velocity of GV  & 0.03~m/s  & <0.03~m/s  & 0.03 & 0.03\tabularnewline
\midrule 
7 & PSF transducer  & 0.10\,hPa & 0.001\,hPa & -- & --\tabularnewline
\midrule 
8 & ATX & 0.3$^{\circ}$ & 0.1$^{\circ}$C  & -- & --\tabularnewline
\midrule 
9 & $\delta q$ & $<$0.3\,hPa & 0.1\,hPa  & (--,~0.3) & (--,~0.1)\tabularnewline
\bottomrule
\end{tabular}

\protect\caption{Elemental contributions to the uncertainty in measurement of horizontal
wind. Entries '--' indicate negligible contribution to uncertainty.\label{tab:Elemental-h}}
\end{table}

\begin{enumerate}
\item \textbf{BDIFR:} 
The uncertainty in BDIFR is assigned to bias because it is likely
a calibration uncertainty and the resolution and stability are much
smaller than this bias. However, a calibration bias in this measurement
does not affect the final wind measurement because the procedure in
...
such errors are
thought to be negligible, so the propagated error for BDIFR is listed
as negligible. The next item considers the determination of sensitivity
...
and instead replaces them with uncertainties arising from the coefficients
\{$e_{0},\, e_{1}$\} in (\ref{eq:betaFunctionForm}). Because the uncertainties in
the coefficients 
resulting from the fit in Sect.~\ref{sub:radome-sensitivity} are highly correlated,
the estimated
bias and random error are those obtained by simulation in that section
using the estimated uncertainties in the coefficients in (\ref{eq:betaFunctionForm}).
...
PSF the effect is negligible. This was tested as for PSF by evaluating
at representative points. A representative result was that the listed
bias in temperature would lead to a typical bias in TASX of about 0.05\%,
leading to a similar percentage change in the value of the lateral component of the
relative wind. This is negligible in comparison to other sources of uncertainty.
\end{enumerate}
