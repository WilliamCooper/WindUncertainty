...
<<table-1-elements, include=FALSE>>=

# these are the values that enter the table, set this way for consistent changes,
# repetitions, and sums:
TE1a <- 0.1;TE1b <- 0.1;TE1d <- 0;TE1e <- 0.05
TE2a <- 0.1;TE2b <- 0.001;TE2d <- 0.04;TE2e <- 0.004
TE3a <- 0.1;TE3b <- 0.1;TE3d <- 0;TE3e <- 0
TE4a <- 0.3;TE4b <- 0.1;TE4d <- 0.02;TE4e <- 0.001
TE5a <- 0.01;TE5b <- 0.007;TE5d <- 0.08;TE5e <- 0.03
TE6a <- 0.03;TE6b <- 0.03;TE6d <- 0.03;TE6e <- 0.03
TE7a <- 0.1;TE7b <- 0.1;TE7d <- 0;TE7e <- 0
TE8a <- 0.3;TE8b <- 0.1;TE8d <- 0;TE8e <- 0
SQ1d <- TE1d^2+TE2d^2+TE3d^2+TE4d^2+TE5d^2+TE6d^2+TE7d^2+TE8d^2
SQ1e <- TE1e^2+TE2e^2+TE3e^2+TE4e^2+TE5e^2+TE6e^2+TE7e^2+TE8e^2
...
1  &
ADIFR transducer  &
\Sexpr{round(TE1a,1)}~hPa  &
\Sexpr{round(TE1b,1)}~hPa  &
$\gg$50 & 
--  &
\Sexpr{round(TE1e,2)}\tabularnewline
 & & [B] &[A]\tabularnewline
\midrule 
2  &
AKRD coefficients  &
\Sexpr{round(TE2a,2)}$^{\circ}$  &
\Sexpr{round(TE2b,3)}$^{\circ}$  &
10 / $\gg$50&
\Sexpr{round(TE2d,2)}  &
\Sexpr{round(TE2e,3)}\tabularnewline
 & & [A]/[B]&[A]\tabularnewline
\midrule 
3  &
BDIFR transducer  &
\Sexpr{round(TE3a,1)}~hPa  &
\Sexpr{round(TE3b,1)}~hPa  &
$\gg$50 &
--  &
--\tabularnewline
 & & [B] &[A]\tabularnewline
...
4  &
QCF transducer  &
\Sexpr{round(TE4a,1)}~hPa  &
\Sexpr{round(TE4b,1)}~hPa  &
$\gg$50 & 
<\Sexpr{round(TE4d,2)}  &
\Sexpr{round(TE4e,3)}\tabularnewline
 & &[A]&[A]&(both)\tabularnewline
\midrule 
5  &
pitch\footnote{for measurement periods short compared to $\Omega_{Sch}/4$}  &
\Sexpr{round(TE5a,2)}$^{\circ}$  &
\Sexpr{round(TE5b,3)}$^{\circ}$  &
 $>50$&
\Sexpr{round(TE5d,2)}  &
\Sexpr{round(TE5e,2)}\tabularnewline
 & &[A]&[A]&(both)\tabularnewline
\midrule 
6  &
GV vertical velocity  &
\Sexpr{round(TE6a,2)}~m/s  &
<\Sexpr{round(TE6b,2)}~m/s  &
 --&
\Sexpr{round(TE6d,2)}  &
<\Sexpr{round(TE6e,2)}\tabularnewline
 & &[B]&[B]\tabularnewline
\midrule 
7  &
PSF transducer  &
\Sexpr{round(TE7a,1)}\,hPa  &
\Sexpr{round(TE7b,1)}\,hPa  &
 $\approx$5&
--  &
--\tabularnewline
 & &[A]&[A]&(both)\tabularnewline
...
8  &
ATX  &
\Sexpr{round(TE8a,1)}$^{\circ}$  &
\Sexpr{round(TE8b,1)}$^{\circ}$C  &
 $\gg$50&
--  &
--\tabularnewline
& &[A]&[A]&(both)\tabularnewline
...
\Sexpr{format(round(sqrt(SQ1d), 2), nsmall=2)}~m\,s$^{-1}$ and an estimated random uncertainty of %0.07
\Sexpr{round(sqrt(SQ1e), 2)}~m\,s$^{-1}$. This estimate applies to measurements without pitch correction,\index{pitch!correction to} and for such measurements the contribution from uncertainty in pitch is the dominant source of uncertainty. With the correction, the bias estimate decreases to 0.06\,m\,s$^{-1}$, so the pitch correction reduces the uncertainty significantly. 
...
\Sexpr{format(round(sqrt(SQ1d),2), nsmall=2)}  &
\Sexpr{round(sqrt(SQ1e),2)}  &
\Sexpr{round(sqrt(SQ1d+SQ1e), 2)}  &
...
<<WICWIGcalc, include=FALSE, cache=TRUE>>=
...
if (ReloadData) {
  ...
  DataC[(DataC$RF == 1) & ((DataC$Time > as.POSIXct("2014-06-06 08:05:00", tz='UTC')) &
                           (DataC$Time > as.POSIXct("2014-06-06 08:40:00", tz='UTC'))),] <- NA
  save(DataC, file='DataFrames/Sect4WICvsWIG.Rdata')  
} else {
  load(file='DataFrames/Sect4WICvsWIG.Rdata')
}
Valid <- !is.na(DataC$GGALT) & DataC$GGALT > 35000*.3048
DataC[!Valid, ] <- NA

@

<<WICWIGdiff, echo=FALSE, warning=FALSE, include=TRUE, fig.scap="Histogram of the differences between the vertical wind measured by the radome-based system and that measured by the gust-pod system.", fig.cap="Histogram of the differences between the vertical wind measured by the radome-based system (WIC) and that measured by the gust-pod system (WI\\_GP) for all 1-Hz measurements from the DEEPWAVE project where the absolute value of the roll was less than 5$^{\\circ}$, flight speed was above 130~m/s, and flight altitude was FL350 (35,000~ft) or above. The density shows the fraction of points per unit interval in the difference WIC-WI\\_GP. There were a total of about 280,000 measurements used for this plot, but some flights and flight periods were excluded where it appeared, from comparison to other IRUs, that the pitch measurement from the gust-pod system was of degraded quality.", fig.height=4, cache=TRUE>>=

#hist(DataC$WIX-DataC$WI_GP, freq=FALSE, xlim=c(-2,2), breaks=100, main='All-Project (excluding obviously bad data)', xlab='WIC-WI_GP [m/s]')
ggplot(data=DataC, aes(WIC-WI_GP)) + geom_histogram(aes(y=..density..), binwidth=0.04, fill=I("lightyellow"), col=I("black"))+xlim(-2.,2.)+geom_density (col='red')+theme_WAC()
APmean <- mean(DataC$WIX-DataC$WI_GP, na.rm=TRUE)
APsd <- sd(DataC$WIX-DataC$WI_GP, na.rm=TRUE)
